 o°. Application de rentrée version 2012 .°o

Ce module de rentrée a été élaboré par Pierre-Edouard MONTABRUN (Hypnoz BDE 2011),
...
2011:
 -Utilisation du ldap de l'école pour rechercher les informations sur l'élève et préremplir
 ...
 -Mise en place d'une meilleur sécurité, avec un login via session mysql
 (seul le numéro de la session est gardé dans les cookie et les variables sessions, c'est pour cela qu'a été créée la table security).
 ...
 -Trouver un moyen de suivre la scolarité des cotisants (césure, année jeune ingénieur, prolongation de scolrité)
  (très utile pour les procurations)
...
2011:
	...
	-Ajout d'entrées  dans la bdd concernant la portable, l'adresse, la date de création
...
etape1.php
	Première étape de l'inscription d'un nouveau membre:
		-se connecte à la bdd et vérifie s'il existe, si c'est le cas il renvoie une description rapide,
...
img_stat_ecole.php
	Contient la fonction    qui crée une image de compteur et inscrit le stats TEM et TSP
	grâce à la table de statistiques. L'image est ensuite enregistrée dans img_cache.

img_stat_sexe.php
	Contient la fonction    qui crée une image de compteur et inscrit le stats garçons et filles
	grâce à la table de statistiques. L'image est ensuite enregistrée dans img_cache.
	
index.php
	Page d'accueil. En fonction de la réponse de hothentic(), renvoie une page d'authentification
	ou la page d'accueil de l'application. Le menu contient un lien vers les stats et la compta uniquement
	si l'utilisateur authentifié est de niveau 2 (bureau)
	
install.php
	Vérifie que la connexion à la bdd est possible et que le module n'est pas déjà installé.
	Si c'est le cas, il créé les tables dans la bdd et renvoie un mesage correspondant au statut
	de l'installation et aux eventuelles erreurs

rib.js
	Contient la fonction verif_clef() qui renvoit la validité de la clef sur etape2.php
...
search.php
	Fournit le résultat de la recherche passée en paramètre $_GET['q']. '_all' renvoie tout le
	contenu de la table, sinon seul les résultats commençant par $_GET['q'] sont renvoyés sous
	forme de tableau
